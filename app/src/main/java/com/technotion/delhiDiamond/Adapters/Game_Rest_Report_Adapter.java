...
public class Game_Rest_Report_Adapter extends RecyclerView.Adapter<Game_Rest_Report_Adapter.viewHolder> {
    ...
    ArrayList<String> card_num =new ArrayList<>();
    ...
    Context context;
    public Game_Rest_Report_Adapter(   ArrayList<String> card_num,ArrayList<String> closing_coins, ArrayList<String> win_or_lose,ArrayList<String> transaction_time,ArrayList<String> game_id, Context context) {
    ...
    @NonNull
    @Override
    public Game_Rest_Report_Adapter.viewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View viewfull = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.rest_game_report,viewGroup,false);
        Game_Rest_Report_Adapter.viewHolder v = new Game_Rest_Report_Adapter.viewHolder(viewfull);
        return v;
    }

    @Override
    public void onBindViewHolder(@NonNull Game_Rest_Report_Adapter.viewHolder holder, final int i) {
      /*  Glide.with(context).asBitmap().load(mIcons.get(i)).into(viewHolder.imageView);
        ...
        holder.card_display_.setText(card_num.get(i));
       ...
       */ /*   holder.card_display_.setImageResource(card_display.get(i));
       ...
       if(Config.com_patti==0){
           ...
           holder.change_coins_text.setText("Commission");
            ...
            if (!win_or_lose.get(i).equals("Players Have Not Played")) {
                holder.view_full_report.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (Config.game_report_game_type == 3) {
                            ...
                            int g = Integer.parseInt(game);
                            String time = transaction_time.get(i);
                            Intent intent = new Intent(context, Past_jantri.class);
                            intent.putExtra("game_ids", g);
                            intent.putExtra("time", time);
                            context.startActivity(intent);
                        }
